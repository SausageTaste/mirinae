...
namespace { namespace task {
    ...
    public:
        ...
        void collect_cmdbuf(std::vector<VkCommandBuffer>& out) const {
            if (env_item_)
    ...
    private:
        void ExecuteRange(enki::TaskSetPartition range, uint32_t tid) override {
            ...
            this->record(cmdbuf_, *reg_, *env_item_, draw_set_, *rp_, *ctxt_);
    ...
    class DrawSky : public mirinae::DependingTask {

    public:
        void init(
            const VkDescriptorSet sky_tex_desc_set,
            const mirinae::IRenPass& rp,
            mirinae::RpCommandPool& cmd_pool,
            mirinae::VulkanDevice& device
        ) {
            ...
            cmd_pool_ = &cmd_pool;
            device_ = &device;
            sky_tex_desc_set_ = sky_tex_desc_set;
        }
    ...
    private:
        void ExecuteRange(enki::TaskSetPartition range, uint32_t tid) override {
            ...
            mirinae::begin_cmdbuf(cmdbuf_);
            this->record(cmdbuf_, sky_tex_desc_set_, *env_item_, *rp_);
            mirinae::end_cmdbuf(cmdbuf_);
        }
        ...
        static void record(
            ...
            const auto proj_mat = ::make_proj(0.1, 1000.0);
        ...
        mirinae::RpCommandPool* cmd_pool_ = nullptr;
        mirinae::VulkanDevice* device_ = nullptr;
        VkDescriptorSet sky_tex_desc_set_ = VK_NULL_HANDLE;
    };


    class DrawDiffuse : public mirinae::DependingTask {
    ...
    public:
        ...
        void ExecuteRange(enki::TaskSetPartition range, uint32_t tid) override {
            ...
            mirinae::begin_cmdbuf(cmdbuf_);
            this->record(cmdbuf_, *env_item_, *rp_);
            mirinae::end_cmdbuf(cmdbuf_);
        }
        ...
        static void record(
            ...
            const auto proj_mat = ::make_proj(0.01, 10.0);
    ...
    class DrawSpecular : public mirinae::DependingTask {
    ...
    public:
        DrawTasks() {
            draw_base_.succeed(this);
            draw_sky_.succeed(this);
            draw_diffuse_.succeed(this);
            draw_specular_.succeed(this);
            fence_.succeed(
                &draw_base_, &draw_sky_, &draw_diffuse_, &draw_specular_
            );
        ...
        void init(
            const VkDescriptorSet sky_tex_desc_set,
            const mirinae::IEnvmapRpBundle& rp,
            entt::registry& reg,
            ...
            draw_base_.init(reg, rp.rp_base(), cmd_pool, device);
            draw_sky_.init(sky_tex_desc_set, rp.rp_sky(), cmd_pool, device);
            draw_diffuse_.init(rp.rp_diffuse(), cmd_pool, device);
            draw_specular_.init(rp.rp_specular(), cmd_pool, device);
        }
        ...
        void collect_cmdbuf(std::vector<VkCommandBuffer>& out) {
            draw_sky_.collect_cmdbuf(out);
            draw_base_.collect_cmdbuf(out);
            draw_diffuse_.collect_cmdbuf(out);
            draw_specular_.collect_cmdbuf(out);
        }
    ...
    private:
        void ExecuteRange(enki::TaskSetPartition range, uint32_t tid) override {
            ...
            if (!timer_.check_if_elapsed(ENVMAP_UPDATE_INVERVAL)) {
                draw_base_.prepare(*ctxt_, nullptr);
                draw_sky_.prepare(*ctxt_, nullptr);
                draw_diffuse_.prepare(*ctxt_, nullptr);
                draw_specular_.prepare(*ctxt_, nullptr);
                return;
            }
            ...
            for (auto e : reg.view<cpnt::Envmap>()) {
                if (auto tform = reg.try_get<cpnt::Transform>(e)) {
                    const auto d = glm::length(tform->pos_ - view_pos);
                    entities.notify(e, d);
                } else {
                    entities.notify(e, glm::length(view_pos));
                }
            }
            if (entities.empty())
                return;
            ...
            if (chosen) {
                ...
                auto& env_cpnt = reg.get<cpnt::Envmap>(entity);
                ...
                if (auto tform = reg.try_get<cpnt::Transform>(entity))
                    ...
                    chosen->world_mat_ = IDENTITY;
            }

            draw_base_.prepare(*ctxt_, chosen);
        ...
        DrawBase draw_base_;
        DrawSky draw_sky_;
        DrawDiffuse draw_diffuse_;
        DrawSpecular draw_specular_;
        ...
        mirinae::EnvmapBundle* envmaps_ = nullptr;
    };
    ...
    public:
        ...
        void init(
            ...
            record_tasks_.init(
                sky_tex_desc_set, rp, reg, envmaps, cmd_pool, device
            );
        }
