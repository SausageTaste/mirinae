...
void main() {
    ...
    if (gl_InvocationID == 0) {
        vec4 p00 = u_pc.pvm_ * gl_in[0].gl_Position;
        vec4 p01 = u_pc.pvm_ * gl_in[1].gl_Position;
        vec4 p11 = u_pc.pvm_ * gl_in[2].gl_Position;
        vec4 p10 = u_pc.pvm_ * gl_in[3].gl_Position;
        p00 /= p00.w;
        p01 /= p01.w;
        p11 /= p11.w;
        p10 /= p10.w;

        const vec2 fbuf_size = u_pc.fbuf_size_;
        ...
        float tess_level0 = min(edge3 * u_pc.tess_factor_, MAX_TESS_LEVEL);
        float tess_level1 = min(edge0 * u_pc.tess_factor_, MAX_TESS_LEVEL);
        float tess_level2 = min(edge1 * u_pc.tess_factor_, MAX_TESS_LEVEL);
        float tess_level3 = min(edge2 * u_pc.tess_factor_, MAX_TESS_LEVEL);
